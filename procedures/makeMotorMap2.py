...
def bootstrapPhiMap(pfi, output, cobras, steps=200):
    """ Take phi map in groups. Used when we do not have a map. """

    phisets = dict()
    for g in 1,2,3:
        gcobras = [c for c in cobras if c.cobraNum%3 == g-1]
        phisets[g] = takePhiMap(pfi, output, gcobras, steps=steps, nameExtra=f'BSgroup{g}')
...
def takePhiMap(pfi, output,
    ...
    dataset = imageSet.ImageSet(pfi, cameraFactory(), output, makeStack=True)

    # record the phi movements
    for n in range(repeat):
        # forward phi motor maps
        dataset.expose(f'phi{nameExtra}BeginForward{n}_')
        ...
        dataset.expose(f'phi{nameExtra}EndForward{n}_')
        ...
        if ontimes is not None:
            ...
            pfi.moveAllSteps(cobras, 0, -5000)
            pfi.calibModel.updateOntimes(*ontimes['normal'])
        dataset.expose(f'phi{nameExtra}End2{n}_')
...
def targetThetasIn(pfi, cobras):
    """ Aim all the theta motors in a module in.  """
...
#    pfi.moveAllSteps(goodCobras, -10000, -5000)
    ...
    cnt = phiSteps//steps
    for n in range(repeat):
        data = fits.getdata(dataPath+f'/phiBegin{n}_0001.fits')
