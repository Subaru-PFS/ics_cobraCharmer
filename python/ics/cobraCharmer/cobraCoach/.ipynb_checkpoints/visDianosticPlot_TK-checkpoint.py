...
def findDesignFromVisit(visit):
    ...
    try:
        ...
        conn = psycopg2.connect("dbname='opdb' host='pfsa-db' port=5432 user='pfs'") 
...
def findRunDir(pfsVisitId):
    search_pattern = f"/data/MCS/202[23456]*/data/PFSC{pfsVisitId:06d}??.fits"
...
def remove_outlier(pos, estCenter, radiusTolerance=0.5):
        medianR = np.median(np.abs((pos-estCenter)))
        indx = np.where(np.abs((pos-estCenter)) < medianR+radiusTolerance)
        filtered_pos=pos[indx]
        return filtered_pos

import numpy as np
from scipy.spatial import KDTree

def uniformize(pos, adjacentTolerance, random_state=None):
    # find clusterings based on FoF
    coords = np.column_stack((pos.real, pos.imag))
    tree = KDTree(coords)
    N = len(pos)
    labels = -np.ones(N, dtype=int)  # Initialize all as isolated (-1)
    ...
    for i in range(N):
        if visited[i]:
            continue
        neighbors = tree.query_ball_point(coords[i], adjacentTolerance)
        if len(neighbors) == 1:
            # Isolated point (only itself)
            visited[i] = True
            continue
        # Otherwise, perform FoF clustering (breadth-first)
        group = set(neighbors)
        ...
        while queue:
            ...
            new_neighbors = tree.query_ball_point(coords[current], adjacentTolerance)
    ...
    for cluster_label in range(cluster_id):
        ...
        chosen = rng.choice(cluster_points)
        selected_indices.append(chosen)

    # Add isolated points:
    isolated_indices = np.where(labels == -1)[0]
    selected_indices.extend(isolated_indices)

    selected_indices = np.array(selected_indices)
    selected = pos[selected_indices]

    return selected

#############################circular fit######################################################
def fit_circle(pos, estCenter):
    if len(pos)<3:
        return np.nan, np.nan, np.nan
    x, y = pos.real, pos.imag
    
    # Initial guess for center (xc, yc) and radius r
    r0 = np.mean(np.sqrt((x - estCenter.real)**2 + (y - estCenter.imag)**2))
    initial_params = np.array([estCenter.real, estCenter.imag, r0])
...
def res_circle(pos, center, radius, avg=True, param_normal=True):
    x, y = pos.real, pos.imag
    
    res = ((x-center.real)**2+(y-center.imag)**2)/radius**2-1
    if avg:
        if param_normal:
            return np.sum(res**2)/(len(x)-3)
        else:
...
class ransac_circle:
    def __init__(self, pos, estCenter, radiusTolerance=0.5, adjacentTolerance=0.03):
        self.estCenter = estCenter
        self.filtered_pos = remove_outlier(pos, estCenter, radiusTolerance)
        self.pos4ransac = uniformize(self.filtered_pos, adjacentTolerance)
        
    def fit(self, ransacTolerance=0.04, n_iter=100, frac_fitdata = 0.75, frac_inliers=0.85, min_fitdata=3, min_inliers=3, save_model=False, remove_cata=True):
        if int(len(self.pos4ransac)*frac_fitdata)<min_fitdata:
            self.best_res = np.nan
            return np.nan+np.nan*1j, np.nan
        
        n_fitdata = int(len(self.pos4ransac)*frac_fitdata)
        self.best_res = np.inf
        best_center, best_radius = np.nan+np.nan*1j, np.nan
        if save_model:
            self.models = {}
        for i in range(n_iter):
            fitdata = np.random.choice(self.pos4ransac, n_fitdata, replace=False)
            fit_center, fit_radius = fit_circle(fitdata, self.estCenter)
            res = res_circle(self.pos4ransac, fit_center, fit_radius, avg=False)
            inliers = np.abs(res)<ransacTolerance
            if np.sum(inliers)>max(int(len(self.pos4ransac)*frac_inliers),min_inliers):
                if remove_cata:
                    if (np.abs(fit_center.real)>250)|(np.abs(fit_center.imag)>250):
                        norm_res = np.nan
                    else:
                        norm_res = res_circle(self.pos4ransac[inliers], fit_center, fit_radius)
                else:
                    norm_res = res_circle(self.pos4ransac[inliers], fit_center, fit_radius)
            else:
                norm_res = np.nan

            if norm_res < self.best_res:
                self.best_res = norm_res
                best_center, best_radius = fit_center, fit_radius
            if save_model:
                model_data = {"data":fitdata, "model":[fit_center, fit_radius], "residual":res, "inliers":np.where(inliers)[0]}
                model_data["normalized_residual"] = norm_res
                self.models[i] = model_data
        return best_center, best_radius

#############################ellipse fit######################################################
def fit_ellipse(pos, estCenter):
    if len(pos)<5:
        return np.nan, np.nan, np.nan
    x, y = pos.real, pos.imag

    # Initial guess: center at mean, a/b from std dev, angle = 0
    a_init = np.abs(np.max(x) - np.min(x)) / 2
    b_init = np.abs(np.max(y) - np.min(y)) / 2
    theta_init = 0.0
    p0 = [estCenter.real, estCenter.imag, a_init, b_init, theta_init]
    ...
    def residuals(params):
        ...
        dx = x - x0
        dy = y - y0
        term1 = ((dx * cos_t + dy * sin_t) / a)**2
        term2 = ((-dx * sin_t + dy * cos_t) / b)**2
        return term1 + term2 - 1  # Ellipse equation error

    result = least_squares(residuals, p0)
    xc, yc, a_, b_, theta = result.x
    ...
    return xc+yc*1j, a+b*1j, theta


def res_ellipse(data, center, radius, theta,  avg=True, param_normal=True):
    ...
    res = (x_rot / radius.real)**2 + (y_rot / radius.imag)**2 - 1
    if avg:
        if param_normal:
            return np.sum(res**2)/(len(x)-5)
        else:
            return np.sum(res**2)/len(x)
    else:
        return res

class ransac_ellipse:
    def __init__(self, pos, estCenter, radiusTolerance=0.5, adjacentTolerance=0.03):
        self.estCenter = estCenter
        self.filtered_pos = remove_outlier(pos, estCenter, radiusTolerance)
        self.pos4ransac = uniformize(self.filtered_pos, adjacentTolerance)
    ...
    def fit(self, ransacTolerance=0.04, n_iter=100, frac_fitdata = 0.75, frac_inliers=0.85, min_fitdata=3, min_inliers=3, save_model=False, remove_cata=True):
        if int(len(self.pos4ransac)*frac_fitdata)<min_fitdata:
            self.best_res = np.nan
            return np.nan+np.nan*1j, np.nan, np.nan
            
        n_fitdata = int(len(self.pos4ransac)*frac_fitdata)
        self.best_res = np.inf
        best_center, best_radius, best_theta = np.nan+np.nan*1j, np.nan, np.nan
        if save_model:
            self.models = {}
        for i in range(n_iter):
            fitdata = np.random.choice(self.pos4ransac, n_fitdata, replace=False)
            fit_center, fit_radius, fit_theta = fit_ellipse(fitdata, self.estCenter)
            res = res_ellipse(self.pos4ransac, fit_center, fit_radius, fit_theta, avg=False)
            inliers = np.abs(res)<ransacTolerance
            if np.sum(inliers)>max(int(len(self.pos4ransac)*frac_inliers),min_inliers):
                if remove_cata:
                    if (np.abs(fit_center.real)>250)|(np.abs(fit_center.imag)>250):
                        ...
                        norm_res = res_ellipse(self.pos4ransac[inliers], fit_center, fit_radius, fit_theta)
                else:
                    norm_res = res_ellipse(self.pos4ransac[inliers], fit_center, fit_radius, fit_theta)
            else:
                norm_res = np.nan
            ...
            if norm_res < self.best_res:
                self.best_res = norm_res
                best_center, best_radius, best_theta = fit_center, fit_radius, fit_theta
            if save_model:
                model_data = {"data":fitdata, "model":[fit_center, fit_radius, fit_theta], "residual":res, "inliers":np.where(inliers)[0]}
                model_data["normalized_residual"] = norm_res
                self.models[i] = model_data
        return best_center, best_radius, best_theta
...
class VisDianosticPlot(object):
    ...
    def visVisitAllSpots(self, pfsVisitID = None, subVisit = None, camera = None, dataRange=None):
        ...
        try:
            db=opdb.OpDB(hostname='pfsa-db', port=5432,dbname='opdb',
                        username='pfs')
        except:     
    ...
    def visStoppedCobra(self, pfsVisitID, tolerance = 0.01, getStoppedNum=False, show=True):
        '''
        ...
        for subID in range(maxIteration+1):
            ...
            db=opdb.OpDB(hostname='pfsa-db', port=5432,
    ...
    def visAllFFSpots(self, pfsVisitID=None, vector=True, vectorLength=0.05, camera = None, 
        ...
        for count, sub in enumerate(range(*dataRange)):
            ...
            for i in range(2):
                transformed = pt.updateTransform(mcsData, fids, matchRadius=4.2,nMatchMin=0.1)
                rfid, rdmin = transformed[0], transformed[1]
            nMatch = sum(rfid > 0)
    ...
    def visAllFFOffsetHisto(self, posData, Iteration = None, Axes = None, binNum = 80, offsetBox = 0.05, dataOnly=False, show=True):
        ...
        if Iteration is not None:
            ...
            if show:
                ...
                n, bins = np.histogram(ffOffset,bins=binNum,range=(0,offsetBox))
        else:
            ffOffset = np.abs(posData - avgPos)
            if show:
                n, bins, patches = ax.hist(ffOffset.flatten(),bins=binNum,range=(0,offsetBox))
            else:
                n, bins = np.histogram(ffOffset.flatten(),bins=binNum,range=(0,offsetBox))
