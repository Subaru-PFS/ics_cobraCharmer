...
class OntimeOptimize:
    ...
    def _searchNextGoodSpeed(self, fiberInx, direction, targetSpeed):
        ...
        if self.nPoints == 1:
            ...
            elif np.abs(ndf[f'{direction}'].values[-1]) <  np.abs(targetSpeed):
    ...
    def _solveForSpeed(self, targetSpeed=None):
        ...
        if len(fastOnes[0]) > 0:
            logging.warning(f'some motors too fast: {fastOnes[0]}: '
        ...
        if len(slowOnes[0]) > 0:
            logging.warning(f'some motors too slow {slowOnes[0]}: '
...
def exploreModuleOntime(arm=None, thetaOnTimeMax = 0.065, phiOnTimeMax=0.05,
    ...
    logger.info('Starting to run on-time optimization')
    ...
    logger.info('Starting to work on optimization based on all data')
